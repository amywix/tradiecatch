...
export default function SettingsScreen() {
  ...
  const DEFAULT_MSGS: Record<string, string> = {
    ...
    service_retry: "Sorry, I didn't catch that. Please reply with a number from 1-{count}:\n\n{menu}",
    other_prompt: "No worries! Please type a brief description of what you need help with and we'll get back to you.",
    urgency_prompt: "Thanks for letting us know.\nIs this an emergency right now?\n\nReply YES if urgent or NO if it can wait.\n\nIf urgent, we'll prioritise your job immediately.",
    urgency_yes: "We're treating this {service} as urgent. Our team will call you ASAP.\n\nWhat's the address so we can head your way?",
    urgency_retry: "Please reply YES if this is urgent, or NO if it can wait.",
    address_request: "Great! {service}.\n\nWhat's the address for the job?",
    out_of_area: "Thanks for the address{distance}. That's outside our usual service area for {service}, so we can't auto-book this one. We've passed your details on — we'll review and get back to you directly.\n\n- {businessName}",
    paid_service_response: "{service} — sorted.\n\nPayment link: {paymentLink}\n\n{followUpNote}\n\n- {businessName}",
    time_preference: "Thanks! And what's the best time:\n1. Morning\n2. Afternoon\n3. ASAP",
    booking_date_intro: "Thanks! What day works best for you?\n\n{menu}",
    booking_date_retry: "Please reply with a number from 1-{count}:\n\n{menu}",
    booking_slot_intro: "Great, {date}!\n\nWhat time suits you?\n\n{menu}",
    booking_slot_retry: "Please reply with a number from 1-{count}:\n\n{menu}",
  ...
  const MSG_LABELS: Record<string, { label: string; hint: string }> = {
    greeting_missed_call: { label: 'Missed Call Greeting', hint: 'First SMS after a missed call. Use {businessName} or {businessLine} ("\\nThis is Name.")' },
    greeting_demo: { label: 'Inbound Text Greeting', hint: 'Sent when someone texts your business number without a prior call. Use {businessName}' },
    service_intro: { label: 'Service Menu', hint: 'Asks which service they need. Use {name} for the customer name and {menu} for the numbered services list' },
    service_retry: { label: 'Service Menu — Invalid Reply', hint: 'Shown if they reply with something that isn\'t a valid menu number. Use {count} and {menu}' },
    other_prompt: { label: '"Other" Service — Describe', hint: 'Shown after they pick "Other" from the service menu. No variables' },
    urgency_prompt: { label: 'Urgency Check', hint: 'Asked when service contains "urgent", "emergency", or "power outage". Use {service}' },
    urgency_yes: { label: 'Urgency Confirmed (YES)', hint: 'Shown when they confirm it\'s urgent. Use {service}' },
    urgency_retry: { label: 'Urgency — Invalid Reply', hint: 'Shown if they don\'t reply YES or NO. No variables' },
    address_request: { label: 'Address Request', hint: 'Asks for the job address. Use {service}' },
    out_of_area: { label: 'Out of Service Area', hint: 'Sent when the address is outside your travel radius. Use {distance} (e.g. " (about 35km away)"), {service}, {businessName}' },
    paid_service_response: { label: 'Paid Service — Send Stripe Link', hint: 'Sent for services that require upfront payment (e.g. Snapchat call). Use {service}, {paymentLink}, {followUpNote}, {businessName}' },
    time_preference: { label: 'Time Preference (Manual)', hint: 'Shown when no booking calendar is configured. Edit the 3 options as needed' },
    booking_date_intro: { label: 'Booking — Pick a Day', hint: 'Shown when booking calendar is on. Use {menu} for the date list' },
    booking_date_retry: { label: 'Booking — Day Invalid Reply', hint: 'Shown if they reply with an invalid day number. Use {count} and {menu}' },
    booking_slot_intro: { label: 'Booking — Pick a Time Slot', hint: 'Shown after they pick a day. Use {date} and {menu}' },
    booking_slot_retry: { label: 'Booking — Slot Invalid Reply', hint: 'Shown if they reply with an invalid slot number. Use {count} and {menu}' },
    booked_manual: { label: 'Booking Confirmation (Manual)', hint: 'Sent when the booking is locked in. Use {dateTime}, {urgentNote}, {businessName}' },
    ...
    followup_complete: { label: 'After-Booking Follow-up', hint: 'Sent if customer replies again after the booking is confirmed. Use {businessName}' },
  };
